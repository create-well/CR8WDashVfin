...
type SyncTrigger = 'manual' | 'schedule' | 'webhook' | 'reconcile';
...
async function recordSyncAudit(
  ...
  // Telemetry must never make an otherwise successful mirror write fail.
  try {
    const sourceRows = args.sourceResults.map(result => ({
      run_id: args.runId,
      source_key: result.source,
      status: result.records ? 'passed' : 'failed',
      observed_count: result.records?.length ?? 0,
      created_count: result.records?.length ?? 0,
      updated_count: 0,
      deleted_count: 0,
      conflict_count: 0,
      source_updated_at: result.records?.reduce<string | null>((latest, record) => (
        record.sourceLastEditedAt && (!latest || record.sourceLastEditedAt > latest)
          ? record.sourceLastEditedAt
          : latest
      ), null),
      mirror_updated_at: args.completedAt,
      ...(result.error ? { error_class: 'source_sync_error' } : {}),
    }));
    const { error } = await db.from('cr8w_sync_runs').upsert({
      run_id: args.runId,
      started_at: args.completedAt,
      completed_at: args.completedAt,
      trigger: args.trigger,
      mode: 'write',
      status: args.status,
      source_count: args.sourceResults.length,
      record_count: args.recordsSeen,
      write_count: args.writes,
      error_count: args.sourceResults.filter(result => result.error).length,
      retry_count: 0,
      lag_seconds: args.latestSourceEdit
        ? Math.max(0, Math.floor((Date.parse(args.completedAt) - Date.parse(args.latestSourceEdit)) / 1000))
        : null,
      error_class: args.status === 'passed' ? null : 'source_sync_error',
      operator_context: 'cr8w-dashboard',
    });
    if (error) return;
    await db.from('cr8w_sync_source_runs').upsert(sourceRows);
  } catch {
    // No raw Notion payloads or credentials are included in this telemetry.
  }
}
...
export async function runSync(
  dryRun: boolean,
  entries: [NotionSourceKey, NotionSourceConfig][] = ENABLED_NOTION_SOURCES,
  ...
  if (!dryRun) {
    ...
    await recordSyncAudit(db, {
      runId,
      trigger,
      status: failed.length === 0 ? 'passed' : successful.length > 0 ? 'partial' : 'failed',
      sourceResults,
      recordsSeen,
      writes: successful.length + 1,
      latestSourceEdit,
      completedAt,
    });
  }
