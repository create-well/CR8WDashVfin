...
- Non-production validation ran on dedicated project `cr8w-dash-v2-rollback-nonprod` (`tcqybqimriafewwwdhoa`): all 13 plan steps passed. Step 12 (advisory-lock concurrency) proven 2026-09-10 ~16:50 local over `psql` with the non-prod DB password: a lock-holding session blocked a concurrent publish for ~5 s, which then committed cleanly; uncontended control ran in 407 ms. The non-prod DB URL is stored locally as `SUPABASE_NONPROD_DB_URL` in `.env.local` for future guarded runs.
...
Using the connected Notion read-only MCP, the provided Create Well OS Master System page was fetched successfully. Its page-linked shared data sources were counted without writing to Notion or Supabase: Skills 26, spine 18, The Well 7, Flow Motion 8, Water Ready 1, Who Holds the Well 8, and Depanty 2. These are connector read tests, not dashboard mirror synchronization counts; they are not registered in `api/notion-sources.ts` and must not be treated as authorized production mirror sources without a source-registry and sensitivity review.
