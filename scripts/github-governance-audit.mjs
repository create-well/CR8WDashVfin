...
function isAuditLogUnavailable(error) {
  const stderr = String(error?.stderr || "");
  const raw = String(error?.stdout || "");
  if (/\(HTTP (403|404)\)/.test(stderr)) return true;
  try {
    const parsed = JSON.parse(raw || "[]");
    const status = String((Array.isArray(parsed) ? parsed[0] : parsed)?.status || "");
...
function fetchRepoEvents() {
  const repoEvents = gh(`repos/${owner}/${name}/events`, ["--method", "GET", "-f", "per_page=100"]);
  return repoEvents.filter((event) => new Date(event.created_at || event.updated_at || 0) >= new Date(since));
}
...
if (auditMode === "org-audit") {
  try {
    ...
    if (!isAuditLogUnavailable(error)) throw error;
    source = "repo-events-fallback";
    events = fetchRepoEvents();
    console.warn("Organization audit log unavailable; falling back to repository events.");
  }
